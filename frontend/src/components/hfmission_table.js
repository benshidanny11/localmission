...
import ManageAction from './hfmanage_btn';
import { useSelector, useDispatch } from 'react-redux';
import { fetchMissionDetails } from '../redux/slices/missionForApprovalSlice';
...
const [rowsPerPage, setRowsPerPage] = useState(10);
  ...
  const handleSearch = (searchTerm) => {
    ...
    const filteredData = missionDetails.filter(row => {
      ...
      const employeeInfo = `${row.employee.givenName} ${row.employee.familyName}`.toLowerCase();
