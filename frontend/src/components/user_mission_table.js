...
import ManageAction from './user_manageBtn';
...
function Mission() {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(10);   //set to shows 10 rows per table
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [dispatch, hasFetched, missionDetails]);
    ...
    <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' , gap: 5}}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <FilterMissionList onFilter={handleFilter} onReset={handleReset} type={1}/>
        </Box>
        <CreateRequest />
      </Box>
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ backgroundColor: '#5982b2' }}>
              ...
              <TableCell sx={{ color: 'white', fontSize: '0.60rem', fontWeight: 'bold', padding: '2px 4px' }}>Action</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
          {loading ? (
            ...
            paginatedRows.length > 0 ? (
              paginatedRows.map((row, index) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell sx={{ fontSize: '0.60rem', padding: '2px 4px' }}>{new Intl.NumberFormat().format(row.missionAllowance)} Rwf</TableCell>
